...
from building_model import Building_Model
...
class Node(ClientNode):
    ...
    def __init__(self, host, input_attributes=None, output_attributes=None, is_first=False):
        ...
        self.Model = Building_Model(init_values["U"],init_values["C"],init_values["T_init"])
    ...
    def step(self, current_time, time_step):
        ...
        for key, value in self.input_values.items():
            ...
            self.redis.rpush('IN||' + self.name + '||' + key, getattr(self.Model, key))
        ...
        logging.debug(self.name, "outputs", {key: getattr(self.Model, key) for key in self.output_attributes})
        for key in self.output_attributes:
            self.update_attribute(key, getattr(self.Model, key))

        # Save output attributes and corresponding simulation time step to Redis DB
        for key in self.output_attributes:
            self.redis.rpush('OUT||' + self.name + '||' + key, getattr(self.Model, key))
